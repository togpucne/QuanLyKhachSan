...
<style>
    ...
    @keyframes pulseGlow {
        ...
        50% {
            box-shadow: 0 4px 25px rgba(0, 104, 255, 0.8),
                0 0 15px rgba(0, 104, 255, 0.6);
        }
    }

    /* Animation quay tròn nhẹ */
    ...
    /* Thêm class cho các hiệu ứng khác nhau */
    .floating-btn.animate-shake {
        animation: shakeStrong 3s ease-in-out infinite;
    }

    .floating-btn.animate-pulse {
        animation: pulseGlow 2s ease-in-out infinite;
    }

    .floating-btn.animate-spin {
        animation: gentleSpin 4s ease-in-out infinite;
    }

    /* Hiệu ứng kết hợp: vừa rung vừa nhấp nháy */
    .floating-btn.animate-combo {
        animation: floatShake 3s ease-in-out infinite,
    ...
    /* Facebook Button - Màu Facebook */
    .floating-facebook {
        background: linear-gradient(135deg, #1877F2, #0D5FAD);
    }
...
<!-- Button đơn giản với hiệu ứng -->
<div class="floating-btn-container">
    <!-- Button Facebook với hiệu ứng rung lắc -->
    <div class="floating-btn floating-facebook animate-combo" onclick="openFacebook()">
        <i class="fa-brands fa-facebook-f"></i>
        <span class="floating-tooltip">Facebook Tỏa Sáng Resort</span>
        <span class="hotline-badge">NEW</span>
    </div>

    <!-- Button Zalo với hiệu ứng khác (tùy chọn thêm) -->
    <!-- <div class="floating-btn floating-zalo animate-shake" onclick="openZalo()">
        <i class="fa-brands fa-zalo"></i>
        <span class="floating-tooltip">Zalo: 0987.654.321</span>
    </div> -->
</div>

<script>
    function openFacebook() {
        window.open('https://www.facebook.com/trong.phuc.53412', '_blank');
    }

    // Tự động thay đổi hiệu ứng sau mỗi 10 giây (tùy chọn)
    setTimeout(function() {
        const btn = document.querySelector('.floating-btn');
        const animations = ['animate-shake', 'animate-pulse', 'animate-spin', 'animate-combo'];
        let currentIndex = animations.indexOf(btn.classList[2]) || 0;

        btn.classList.remove(animations[currentIndex]);
        currentIndex = (currentIndex + 1) % animations.length;
        btn.classList.add(animations[currentIndex]);
    }, 10000);
</script>
<!--  -->
<!-- Banner Carousel -->
